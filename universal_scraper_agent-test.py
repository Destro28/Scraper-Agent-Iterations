...
DOWNLOAD_DIR = "./pdfs"
MAX_PAGES = 500        # Safety limit on pages to visit
...
async def detect_selectors(html_snippet):
    ...
    try:
        ...
        if not json_match:
            logger.warning(f"No JSON found in API response: {result_str}")
            return []
...
async def main():
    ...
    while queue and len(visited_pages) < MAX_PAGES and len(downloaded_pdfs) < MAX_PDFS:
        ...
        try:
            ...
            # **FIX APPLIED HERE**
            # Reduced HTML size to prevent overloading the local model.
